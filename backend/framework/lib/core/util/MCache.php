...
function ob_callback() {
	...
	$page_location = BACKEND_ROOT . '/' . APPLICATION . '/' . APPLICATION_CACHE . '/' . $page_uuid;
...
class MCache {
	...
	public static function cache($minutes) {
		...
		if (file_exists(BACKEND_ROOT . '/' . APPLICATION . '/' . APPLICATION_CACHE . '/' . $page_uuid)) {

			$creation_time = filemtime(BACKEND_ROOT . '/' . APPLICATION . '/' . APPLICATION_CACHE . '/' . $page_uuid);

			if (time() - $creation_time < $minutes * 60) {
				echo file_get_contents(BACKEND_ROOT . '/' . APPLICATION . '/' . APPLICATION_CACHE . '/' . $page_uuid);
				exit();
			}
